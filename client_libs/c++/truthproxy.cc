...
void TruthProxy::FillData(player_msghdr_t hdr, const char* buffer)
{
  ...
  x = (double)(int)ntohl(((player_truth_data_t*)buffer)->px)  / 1000.0;
  y = (double)(int)ntohl(((player_truth_data_t*)buffer)->py) / 1000.0;
  // heading in NBO integer degrees to double radians
  a = DTOR((double)(int)ntohl(((player_truth_data_t*)buffer)->pa));
}
...
int TruthProxy::GetPose( double *px, double *py, double *pa )
{
  ...
  *px = (double)(int)ntohl(config.px) / 1000.0;
  *py = (double)(int)ntohl(config.py) / 1000.0;
  *pa = DTOR((double)(int)ntohl(config.pa));
...
int TruthProxy::SetPose( double px, double py, double pa )
{
  ...
  config.pa = htonl((int) (fmod( RTOD(pa) + 360.0,  360.0 ))); 
