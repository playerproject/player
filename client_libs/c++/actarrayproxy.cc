...
void ActArrayProxy::Print()
{
  ...
  {
    printf (" %2d |\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%s\n", ii, (actuatorGeom[ii].type ? "Linear" : "Rotary"),
            actuatorGeom[ii].min, actuatorGeom[ii].centre, actuatorGeom[ii].max, actuatorGeom[ii].home, actuatorGeom[ii].configSpeed,
            actuatorData[ii].position, actuatorData[ii].speed, actuatorData[ii].state, (actuatorGeom[ii].hasBrakes ? "Y" : "N"));
  }
}
