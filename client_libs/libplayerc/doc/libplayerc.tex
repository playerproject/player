...
\newpage
\section{The {\tt wifi} proxy}

The {\tt wifi} proxy is used to query the state of a wireless network.
It returns information such as the link quality and signal strength of
access points or of other wireless NIC's on an ad-hoc network.
\begin{quote}
\lstinputlisting{wifi.f.tex}
\end{quote}
The {\tt create} function returns a pointer to a {\tt playerc\_wifi\_t}
structure, which has the following elements:
\begin{quote}
\lstinputlisting{wifi.t.tex}
\end{quote}
