...
def make_tex(filename, section, blocks):
    ...
    while index < len(blocks):
        ...
        if block.type == None:
            ...
            index = make_function(texfile, section, blocks, index)
        else:
            index += 1
...
def make_struct(file, section, blocks, index):
    ...
    name = string.replace(block.name[1:], '_', '\\_')
...
def make_function(file, section, blocks, index):
    """Generate function blocks."""

    block = blocks[index]

    file.write('\\begin{minipage}[t]{\\columnwidth}\n')
    for line in block.code:
        line = string.replace(line, '_', '\\_')
        line = string.strip(line)
        if len(line) == 0:
            continue
        file.write('{\\small \\bf %s}\n' % line)
    file.write('\\end{minipage}\n')
