...
USAGE = "USAGE: playerinterfacegen.py [%s | %s | %s]  [<interface file>|<interface dir>]" % (HEADER_MODE, FUNCTIONTABLE_MODE, UTILS_MODE)
...
""" % {
  ...
  "interface_comment" : interface_comment}  
    ...
    for m in interface_messages:
      ...
      print "    (player_pack_fn_t)%(dt_base)s_pack, (player_copy_fn_t)%(dt)s_copy, (player_cleanup_fn_t)%(dt)s_cleanup}," % { "dt_base": m.datatype[:-2], "dt": m.datatype}
