...
// Esta funciÛn NO debe declararse como est·tica.
...
static void InicializarDS(float dsmax,float dsmin) {
  ...
  for (i=1; i<SECTORES/2; i++) {
    ...
    robot.ds[SECTORES-i]=robot.ds[i]; // El robot es simÈtrico respecto del eje X.
  }
...
static int ParadaEmergencia(TInfoND *nd) {
  // Devuelve 1 si hay peligro de colisiÛn y hay que hacer una parada de emergencia;
  // devuelve 0 en caso contrario.
  // En la detecciÛn de colisiÛn se tiene en cuenta que el robot es simÈtrico respecto del eje X.
...
static void SiguienteDiscontinuidad(TInfoND *nd,int principio,int izquierda,int *discontinuidad,int *ascendente) {
  // Se busca desde "principio" en la direcciÛn indicada por "izquierda".
...
static int ObjetivoAlcanzable(TInfoND *nd,TRegion *region,int direccion_tipo) {
  ...
  TCoordenadas objetivo_intermedio;                // Respecto de un SR con origen en el origen de SR1 y girado hasta que el semieje positivo de abscisas coincide con la direcciÛn al objetivo intermedio.
...
static void SeleccionarRegion(TInfoND *nd) {
  ...
  if (objetivo_a_la_vista) {
    ...
    if (!region->principio_ascendente && !region->final_ascendente) {
      ...
      indice_auxiliar=nd->regiones.longitud; // No incrementamos la longitud del vector: Nuestra regiÛn auxiliar es ilegal.
      region_auxiliar=&(nd->regiones.vector[indice_auxiliar]);

      *region_auxiliar=*region; // Utilizamos la regiÛn auxiliar para almacenar el contenido de la regiÛn que vamos a modificar.
      ...
      nd->regiones.longitud++; // Regularizamos la situaciÛn de nuestra regiÛn auxiliar y
      indice=indice_auxiliar;  // la escogemos como regiÛn a examinar.
  ...
  do {
    ...
    if (distancia_izquierda<=distancia_derecha) {
      ...
      // Probamos por la regiÛn izquierda.
      ...
      // Probamos por la regiÛn derecha.
...
static void ObtenerObstaculos(TInfoND *nd,float beta) {
  // Buscamos todos los obst·culos que estÈn dentro de la distancia de seguridad y nos quedamos
...
static void control_angulo(TInfoND *nd) {
  // C·lculo del ·ngulo de movimiento en funciÛn de la regiÛn escogida para el movimiento del robot, la situaciÛn del objetivo y,
  // en su caso, la distancia a los obst·culos m·s prÛximos. 
                      ...
                      TInfoEntorno *mapa,void *informacion) 
{
  ...
  // Devuelve NULL si se requiere una parada de emergencia o si no encuentra una regiÛn por la que hacer avanzar el robot.
...
//depuracion=fopen("depuracion.txt","at");
  ...
  // SectorizaciÛn del mapa.

  SectorizarMapa(mapa,&nd);

  // EvaluaciÛn de la necesidad de una parada de emergencia.
  ...
  // Deteccion de fin de trayecto. -- DespuÈs de considerar la necesidad de una parada de emergencia.
  ...
  control_angulo(&nd); // ObtenciÛn de la direcciÛn de movimiento.
  control_velocidad(&nd); // ObtenciÛn de la velocidad de movimiento.
...
/*       printf("Entra en cutting %d\n",robot.geometriaRect); */
      Cutting(&nd,&velocidades); // Evitar colisiÛn en zona posterior.
    }
  }
...
/*     printf("w = %f\n",velocidades.w);   */
  ...
  // Copia (si se requiere) de la informaciÛn interna de ND para que quede accesible desde el exterior.
  
  if (informacion)
    *(TInfoND*)informacion=nd;
  
  // DevoluciÛn de resultados.
