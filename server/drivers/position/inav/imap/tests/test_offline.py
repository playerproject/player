...
class MapTest:
    ...
    def update_map(self, datatime, odom_pose, laser_ranges):
        ...
        if self.fit:
            pose = self.est_pose
            (pose, err) = self.map.fit_ranges(pose, self.laser_pose, laser_ranges)
            self.est_pose = pose
...
def main(logfile, fit, scale, skip, noise, export):
    ...
    try:
        ...
        while 1:
            ...
            if type == 'position' and index == 0:  # HACK
                ...
                print odom_time, laser_time

                # Update the map
                map.update_map(laser_time, odom_pose, laser_ranges)
...
if __name__ == '__main__':
    ...
    if not prof:
        main(logfile, fit, scale, skip, noise, export)
    else:
        profile.run('main(logfile, fit, scale, skip, noise, export)', '.profile')
