...
///////////////////////////////////////////////////////////////////////////
// Constructor
CPositionModel::CPositionModel( char* name, char* type, 
...
void CPositionModel::VelocityControl( void )
{
  switch( this->steer_mode )
    ...
    default:
      PRINT_WARN2( "position model %s unknown steer mode (%d)", 
		   this->name, this->steer_mode );
      break;
  }   
}
...
{  
  ...
  //PRINT_WARN1( "updating position %s", this->name );
  
  // if we're on, update the robot's position
  if( this->power_on )
    {
      //PRINT_WARN1( "position %s is powered up and updating", this->name );
	...
	default:
	  PRINT_WARN2( "position model %s has unrecognized control mode %d",
		      this->name, this->control_mode );
	  break;
	}
...
void CPositionModel::PositionControl()
{
  ...
  switch( this->steer_mode )
    ...
    default:
      PRINT_ERR2( "position model %s has unknown steer mode %d", 
		  this->name, this->steer_mode );
...
int CPositionModel::RtkStartup( rtk_canvas_t* canvas )
{
