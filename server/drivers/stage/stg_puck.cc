...
// Default constructor
//
CPuck::CPuck( char* name, char* type, char* color, CEntity *parent)
...
int CPuck::Update( void )
{
  ...
  if(m_parent_entity != (CEntity*)CEntity::root ) // root is the top-level entity
  ...
  if( (CEntity::simtime - m_last_update > m_interval) 
      && (m_parent_entity == (CEntity*)CEntity::root) )
    {	
...
void CPuck::PuckMove()
{
  ...
  if(m_parent_entity != (CEntity*)CEntity::root ) return;
