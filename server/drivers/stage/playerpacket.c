...
void PlayerSonarData( player_sonar_data_t* player, double* ranges, size_t len )
{
  assert( player );
  
  int num_ranges = len / sizeof(double);
  
  player->range_count = htons((uint16_t)num_ranges);
  
  // Store range in mm
  int s;
  for (s = 0; s < num_ranges; s++)
    player->ranges[s] = MM_U16(ranges[s]);
}
...
void PlayerPositionData(  player_position_data_t* player,
			  stage_position_data_t* stage )
{
  // Compute odometric pose
  // Convert to mm and degrees (0 - 360)
  // commands for velocity control
  player->xspeed = MM_32(stage->xdot);
  player->yspeed = MM_32(stage->ydot);
  player->yawspeed = Deg_32(stage->adot);  
  
  // commands for position control
  player->xpos = MM_32(stage->x);
  player->ypos = MM_32(stage->y);
  player->yaw = Deg_32(stage->a); 
  
  player->stall = 0;

  //printf( "DATA RAW %.2f %.2f %.2f\n", stage->x, stage->y, stage->a );
  //printf( "DATA PACKED %ld %ld %ld\n", 
  //  ntohl(player->xpos), ntohl(player->ypos), ntohl(player->yaw) );
