...
{
  Lock();
  player_sonar_data_t* SonarDest = ((player_sonar_data_t*)dest);
  SonarDest->range_count = sonar_count_section;
  for (int i = 0; i < sonar_count_section; ++i)
  	SonarDest->ranges[i] = ((player_rflex_data_t*)device_data)->sonar->[i+sonar_base];
