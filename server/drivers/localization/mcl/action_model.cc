...
/*
 *  Action model for Monte-Carlo Localization.
 */
...
{
    float d_rot1 = (float) atan2( to.y-from.y, to.x-from.x ) - D2R(from.a);
    float d_trans = (float) sqrt( SQR(to.x-from.x) + SQR(to.y-from.y) );
    float d_rot2 = (float) D2R(to.a) - D2R(from.a) - d_rot1;
    ...
    double nd_trans = d_trans + this->normal(this->a3*d_trans + this->a4*abs_d_rot + 100);
